...
async def comprar(update: Update, context):
    ...
    if qr and link:
        usuarios[user_id] = {"username": update.effective_user.username, "confirmado": False, "pix_link": link, "chat_id": chat_id}
        await update.message.reply_photo(photo=qr, caption=f"💰 Pague {VALOR:.2f} via Pix\nChave: {PIX_CHAVE}\nTXID: {user_id}\n\nAguarde a confirmação automática...")
    else:
...
async def expfy_webhook(request):
    ...
    if txid and data.get("status") == "PAID":
        ...
        if user_id in usuarios and not usuarios[user_id]["confirmado"]:
            ...
            await tg_app.bot.send_message(chat_id=usuarios[user_id]["chat_id"], text=f"✅ Pagamento confirmado!\nAcesso: {LINK_GRUPO}")
